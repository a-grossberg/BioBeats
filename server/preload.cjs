...
async function downloadAndExtract(datasetId) {
  ...
  try {
    ...
    // Verify - check multiple possible structures
    const possiblePaths = [
    ...
    for (const imagesPath of possiblePaths) {
      if (existsSync(imagesPath)) {
        const files = readdirSync(imagesPath).filter(f => f.endsWith('.tif') || f.endsWith('.tiff'));
        if (files.length > 0) {
          foundPath = imagesPath;
          console.log(`✓ Extracted ${datasetId}: ${files.length} frames found in ${imagesPath}`);
          break;
        }
      }
    }
    ...
    if (!foundPath) {
      console.log(`⚠ Warning: No images found for ${datasetId}. Checked: ${possiblePaths.join(', ')}`);
    }
  } catch (error) {
